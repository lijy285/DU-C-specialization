...
board_t * makeBoard(int w, int h, int numMines) {
  //allocate the memory fot the board
  board_t * b = malloc(sizeof(* b));
  if (b == NULL) {
    fprintf(stderr,"Out of memory allocating board\n");
    exit(EXIT_FAILURE);
  }
  b->width = w;
  b->height = h;
  b->totalMines = numMines;
  //allocate memory for the rows of the new board
  b->board = malloc(sizeof(int *) * h);
  if (b->board == NULL) {
    fprintf(stderr,"Out of memory allocating board rows\n");
    exit(EXIT_FAILURE);
  }

  //allocate memory for each column in each row and initialize to UNKNOWN
  for(int y = 0; y < h; y++){
    b->board[y] = malloc(sizeof(int *) * w);
    if (b->board[y] == NULL) {
      fprintf(stderr,"Out of memory allocating board columns\n");
      exit(EXIT_FAILURE);
    }
    for(int x = 0; x < w; x++) {
      b->board[y][x] = UNKNOWN;
...
int countMines(board_t * b, int x, int y) {
  int count = 0;
  //iterate over the 8 squares around (x,y)
  ...
  //return the count of mines found
  return count;
}
...
int checkWin(board_t * b) {
  //Won when no unknown squares left
  for (int y = 0; y < b->height; y++) {
    for (int x = 0; x < b->width; x++) {
      if (b->board[y][x] == UNKNOWN) {
  ...
  // if we get here, all squares are known
  return 1;
}
